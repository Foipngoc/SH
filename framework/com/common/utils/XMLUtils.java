...
public class XMLUtils {
     ...
     */
    public static Map<String, Object> xml2Map(File file) {
        try {
            return xml2Map(new FileInputStream(file));
        } catch (FileNotFoundException e) {
            log.error("xml to map error:{}", e.getMessage(), e);
        }
        return Collections.EMPTY_MAP;//返回一个空map
    }
     ...
     * @return
     */
    private static Object xml2Map(Element element, Map<String, Object> map) {
        //加入属性
...
//        System.out.println(element);
        ...
        if (elements.size() == 0) {//没有子节点即标签内文本设置key为value
            map.put("value", element.getTextTrim());
//            if (!element.isRootElement()) {
//                return element.getTextTrim();
//            }
        } else if (elements.size() == 1) {
            Map<String, Object> tm = new HashMap<>();
            map.put(elements.get(0).getName(), xml2Map(elements.get(0), tm));
        } else if (elements.size() > 1) {
            ...
            for (Map.Entry<String, Element> entry : entrySet) {
                ...
                if (elements2.size() > 1) {
                    ...
                    // 同名的数量不大于1则直接递归去
                    Map<String, Object> tm = new HashMap<>();
                    map.put(entry.getKey(), xml2Map(elements2.get(0), tm));
                }
            }
        }
    ...
    private static void map2Xml(Map<String, Object> map, Element element) {
        ...
        for (Map.Entry<String, Object> entry : entries) {//迭代map
            if (entry.getValue() instanceof String) {//如果value是string直接写入
                if("value".equals(entry.getKey())){//如果key是value则直接添加到text
                    element.addText(String.valueOf(entry.getValue()));
                }else{
                    element.addAttribute(entry.getKey(),String.valueOf(entry.getValue()));
                }
                ...
                for (Object o : values) {//迭代list value
                    if (o instanceof String) {//如果是string 直接写入
//                        element.addElement(entry.getKey()).addText(String.valueOf(o));
    ...
    public static void main(String[] args) throws IOException, DocumentException {
        String str = "<XX><a type='1' id='123'>111</a><a type='2'>2</a></XX>";

//        Map<String, Object> map = xml2Map(new File("C:\\Users\\zoumy\\Desktop\\t_Accessory.xml"));//xml2map
        Map<String, Object> map = xml2Map(str);
        System.out.println(map);

        //取值
        Map<String, Object> root = (Map<String, Object>) map.get("XX");
        List fields = (List) root.get("a");
        String strv = String.valueOf(((Map) fields.get(0)).get("value"));
        String strt = String.valueOf(((Map) fields.get(0)).get("type"));
        System.out.println(strt);
        System.out.println(strv);

        ((Map) fields.get(0)).remove("id");
        ((Map) fields.get(1)).put("id","456");


        System.out.println(map2Xml(map));//map2mxl
        System.out.println(map2Xml(map, new File("C:\\Users\\zoumy\\Desktop\\text.xml")));
        System.out.println(xml2Map(new File("F:\\templog\\201609131405--123456.xml")));
    }
}
